...
from denoising_diffusion_pytorch_1d import RandomOrLearnedSinusoidalPosEmb, SinusoidalPosEmb
...
class Layer(nn.Module):
    ...
    def forward(self, tokens, annotation, mask = None):
        ...
        # print(tokens.shape)
        # print(narrow_out.shape)
        # print(wide_out.shape)
        # print(global_info.shape)
        # print(global_linear_attn)
...
class ProteinBERT(nn.Module):
    def __init__(
        ...
        *,
        ...
        num_global_tokens = 1,
        glu_conv = False,
        # From denoising_diffusion_pytorch_1d.py
        ...
        self.channels = num_tokens
        self.self_condition = self_condition
        input_channels = self.channels * (2 if self_condition else 1)

        # self.token_emb = nn.Embedding(num_tokens, dim) # Replacing the nn.Embedding with a Conv1D, which will also work with not one-hot encoded data (such as noise)
    ...
    def forward(self, x, time, x_self_cond = None, annotation = None, mask = None):
        b =  x.shape[0]
        ...
        # instead of x = self.init_conv(x) in U-Net model
        tokens = self.token_emb(x)
        ###
        if annotation is None:
            annotation = torch.zeros((b, self.num_annotation * self.num_global_tokens), device = x.device, dtype = x.dtype)
        ...
        annotation = self.to_annotation_logits(annotation)
        return tokens # , annotation # annotation will be implemented later down the line. For now, we just want to get the model to work
